...
// Tests structure of the json is the way we expect
function test(){
	return axios
		.get(
		...
		.then( ({ data }) => {
			...
			if (typeof first.is_potentially_hazardous_asteroid === 'undefined'){
				throw new Error(
					'Missing key "is_potentially_hazardous_asteroid" from the first data point, presuming remaining data is wrong.'
				);
			}
		});
}
